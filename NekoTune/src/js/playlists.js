...
class PlaylistManager {
  constructor(player, app) {
    this.player = player;
    this.app = app;
    this.activeDownloadRequests = new Map();
    ...
    this.socket.on("collab_sync_playlist", (data) =>
      this.handleCollabMessage({ type: "SYNC_PLAYLIST", ...data }),
    );
    this.socket.on("collab_request_playlist", (data) =>
      this.handleCollabMessage({ type: "REQUEST_PLAYLIST", ...data }),
    );

    // Join rooms for all collaborative playlists we already have
    this.playlists
      .filter((p) => p.isCollaborative && p.shareCode)
      .forEach((p) => {
        this.socket.emit("join_session", {
          sessionCode: "collab_" + p.shareCode,
          user: { name: "peer" },
        });
      });
  ...
  updateFavRemoveButton() {
    const selectedCount = document.querySelectorAll(
      "#favorites-list .track-item .track-checkbox:checked",
    ).length;
  ...
  initEvents() {
    ...
    document
      .getElementById("btn-create-playlist")
      .addEventListener("click", () => {
        this.showCreatePlaylistModal();
      });

    document
      .getElementById("btn-confirm-create-playlist")
      .addEventListener("click", () => {
        this.createPlaylist();
      });
    ...
    if (btnJoinUI) {
      btnJoinUI.addEventListener("click", () => {
        ...
        document
          .getElementById("join-playlist-modal")
    ...
    if (btnConfirmJoin) {
      btnConfirmJoin.addEventListener("click", () => {
        const code = document
          .getElementById("join-playlist-code")
          .value.trim()
          .toUpperCase();
        if (code) {
    ...
    document
      .getElementById("btn-manage-favorites")
      ?.addEventListener("click", () => {
        this.isFavManageMode = true;
        document.getElementById("btn-manage-favorites").style.display = "none";
        document
          .getElementById("favorites-bulk-actions")
          .classList.remove("hidden");
        document.getElementById("btn-fav-select-all").style.display =
          "inline-block";
    ...
    document
      .getElementById("btn-fav-cancel-manage")
      ?.addEventListener("click", () => {
        this.isFavManageMode = false;
    ...
    document
      .getElementById("btn-fav-select-all")
      ?.addEventListener("click", () => {
        document
          .querySelectorAll("#favorites-list .track-item .track-checkbox")
          .forEach((cb) => {
            cb.checked = true;
          });
        document.getElementById("btn-fav-select-all").style.display = "none";
        document.getElementById("btn-fav-deselect-all").style.display =
          "inline-block";
        this.updateFavRemoveButton();
      });

    document
      .getElementById("btn-fav-deselect-all")
      ?.addEventListener("click", () => {
        document
          .querySelectorAll("#favorites-list .track-item .track-checkbox")
          .forEach((cb) => {
    ...
    document
      .getElementById("btn-fav-remove-selected")
      ?.addEventListener("click", () => {
        ...
        this.favorites = this.favorites.filter(
          (t) => !idsToRemove.includes(typeof t === "string" ? t : t.id),
        );
        this.saveFavorites();
    ...
    document
      .getElementById("btn-play-playlist")
      .addEventListener("click", () => {
        this.playCurrentPlaylist();
      });

    document
      .getElementById("btn-shuffle-playlist")
      .addEventListener("click", () => {
        this.shuffleCurrentPlaylist();
      });

    document
      .getElementById("btn-edit-playlist")
      .addEventListener("click", () => {
        this.editCurrentPlaylist();
      });

    document
      .getElementById("btn-delete-playlist")
      .addEventListener("click", () => {
  ...
  createPlaylist(name, desc) {
    ...
    const inputDesc =
      desc || document.getElementById("new-playlist-desc").value.trim();
    ...
    if (playlist.isCollaborative) {
      this.app.showToast(
        `Playlist collaborativa creata! Codice: ${playlist.shareCode}`,
      );
  ...
  handleCollabMessage(data) {
    if (data.type === "SYNC_PLAYLIST") {
      ...
      if (idx !== -1) {
        ...
        this.app.showToast(
          `La playlist "${pData.name}" è stata aggiornata da remoto.`,
        );
      } else {
        ...
        this.renderSidebarPlaylists();
        this.app.showToast(
          `Unito alla playlist collaborativa "${pData.name}"!`,
        );
      }
  ...
  editCurrentPlaylist() {
    ...
    if (isEditing) {
      ...
      document
        .getElementById("btn-edit-playlist")
        .querySelector(".material-icons-round").textContent = "edit";
    } else {
      ...
      document
        .getElementById("btn-edit-playlist")
        .querySelector(".material-icons-round").textContent = "check";
    }
  }
  ...
  toggleFavorite(track) {
    ...
    const idx = this.favorites.findIndex(
      (t) => (typeof t === "string" ? t : t.id) === track.id,
    );
    if (idx > -1) {
  ...
  showPlaylistDetail(playlistId) {
    ...
    document.getElementById("playlist-detail-desc").textContent =
      ...
      item.addEventListener("click", (e) => {
        ...
        if (options.selectable) {
          ...
          if (this.updateFavRemoveButton && containerId === "favorites-list") {
            ...
            const allChecked =
              document.querySelectorAll(
                "#favorites-list .track-item .track-checkbox:not(:checked)",
              ).length === 0;
            if (allChecked) {
              document.getElementById("btn-fav-select-all").style.display =
                "none";
      ...
      if (dlBtn) {
        storage.hasTrack(track.id).then((has) => {
          if (has) {
            ...
            dlBtn.querySelector(".material-icons-round").textContent =
              "offline_pin";
          }
        });
        ...
        dlBtn.addEventListener("click", async (e) => {
          ...
          const activeDownload = storage.getDownloadProgressMap()[track.id];

          if (isDownloaded) {
            await storage.deleteTrack(track.id);
            dlBtn.classList.remove("downloaded");
            icon.textContent = "download";
            storage.removeDownloadProgress(track.id);
            this.updateDownloadButtonState(track.id, "idle");
            this.app.showToast("Rimosso dai download");
          } else if (activeDownload) {
            this.cancelDownload(track.id);
          } else {
            this.updateDownloadButtonState(track.id, "downloading");
            try {
              ...
              if (
                ...
                try {
                  ...
                  downloadUrl = await window.electronAPI.getStreamUrl(
                    track.videoId,
                    quality,
                  );
                  if (!downloadUrl) {
                    throw new Error(
                      "Impossibile ottenere il link di streaming da YouTube",
                    );
                  }
                } catch (ytError) {
                  console.error("YouTube stream error:", ytError);
                  throw new Error("Errore nel recupero del brano da YouTube");
                }
              } else if (track.src) {
                // Use track.src if available
                downloadUrl = track.src;
              } else {
                throw new Error("URL del brano non disponibile");
              }
              ...
              storage.setDownloadProgress(track.id, {
                progress: 0,
                status: "downloading",
                meta: track,
              });
              ...
              storage.removeDownloadProgress(track.id);
              this.updateDownloadButtonState(track.id, "done");
              ...
              if (err.message === "Download annullato") {
                this.app.showToast("Download bloccato");
              } else {
                const errorMsg = err.message || "Errore nel download";
                this.app.showToast(errorMsg);
              }
            }
          }
        });
  ...
  downloadTrackBlob(downloadUrl, track, onProgress) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      this.activeDownloadRequests.set(track.id, xhr);
      ...
      xhr.onload = () => {
        this.activeDownloadRequests.delete(track.id);
        if (xhr.status >= 200 && xhr.status < 300) {
          const blob = xhr.response;
          if (!blob || blob.size === 0) {
            reject(new Error("File scaricato è vuoto"));
            return;
          }
          resolve(blob);
        } else {
  ...
  cancelDownload(trackId) {
    const request = this.activeDownloadRequests.get(trackId);
    if (request) {
      request.abort();
    } else {
      storage.removeDownloadProgress(trackId);
    }
    this.activeDownloadRequests.delete(trackId);
    this.updateDownloadButtonState(trackId, "idle");
  }

  updateDownloadButtonState(trackId, state) {
    const buttons = document.querySelectorAll(
      `.btn-download[data-track-id="${trackId}"]`,
    );

    buttons.forEach((button) => {
      const icon = button.querySelector(".material-icons-round");
      button.classList.remove("spinning-fast", "downloaded");
  ...
  isFavorite(trackId) {
    return this.favorites.some(
      (t) => (typeof t === "string" ? t : t.id) === trackId,
    );
  }
}
